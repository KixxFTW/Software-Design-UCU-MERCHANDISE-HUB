...
DB_CONFIG = {
    ...
    'port': 5432,
    'user': 'postgres.rvbmzkxqvwjjzjhejvhf',
...
def _merch_category_column(cursor) -> str:
    ...
    """
    cursor.execute("SELECT column_name FROM information_schema.columns WHERE table_name = 'merchandise'")
...
def process_order():
    try:
        ...
        try:
            ...
            cursor.execute(
                f"INSERT INTO orders ({columns_sql}) VALUES ({placeholders}) RETURNING id",
                tuple(insert_vals),
            )
            ...
            try:
                if 'student_id' in session:
                    cursor.execute("DELETE FROM cart_items WHERE student_id = %s", (buyer_id,))
                else:
                    cursor.execute("DELETE FROM cart_items WHERE instructor_id = %s", (buyer_id,))
                conn.commit()
            except Exception as cart_err:
                print(f"[process_order] Cart clear warning: {cart_err}")
    ...
    try:
        ...
        query = """
            ...
            WHERE o.student_id = %s
              AND NOT (LOWER(o.status) = 'completed' AND o.payment_status = 'Success')
            ORDER BY o.created_at DESC, o.id DESC
        """
        cursor.execute(query, (student_id,))
